...
if(__name__=='__main__'):
    ...
    labels = ['A', 'B','C','D','E','F','G','K','I','J','L','M','N','O','P','Q','R','S','T','U']
    ...
    colors = colors[:L]; labels = labels[:L];
    tmdir = '/Volumes/HD/network_clustering/PT/Transportation_matrix/'
    
    # Load the Transportation matrix file
    if(type(sp)==str):
        data = np.load(tmdir + 'TMglobal_bin'+str(ddeg)+'_dd'+str(int(dd)) +'_sp'+sp+"_res"+str(res) + '.npz')
    else:
        data = np.load(tmdir + 'TMglobal_bin'+str(ddeg)+'_dd'+str(int(dd)) +'_sp'+str(int(sp))+"_res"+str(res) + '.npz')
    TM = data['TM'][:]
    Lons = data['Lons'][:]
    Lats = data['Lats'][:]
    ...
    B.add_nodes_from(surface_boxes, bipartite=0) # Add the surface boxes as nodes
    B.add_nodes_from(bottom_boxes, bipartite=1) # Add the bottom boxes as nodes
